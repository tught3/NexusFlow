...
### 작업 시작 전 필수
`python E:\FluxStudio\.fluxos\pipeline\task_queue.py --input "<지시 내용>" --project "NexusFlow" --source "nexusflow"`
...
## 세션 충돌 방지 및 파일 소유권
- 다른 세션이나 사용자가 이미 진행 중인 명령, 빌드, 테스트, 파일 수정이 있으면 그 작업을 최우선으로 보호한다.
- 어떤 파일을 수정하기 전에는 `git status --short`, 해당 파일의 diff, 실행 중인 관련 프로세스/테스트/빌드를 먼저 확인한다.
- 수정하려는 파일이 다른 세션에서 이미 dirty 상태이거나 사용 중인 것으로 보이면 무조건 대기하고, 현재 세션에서는 읽기 전용 조사나 다른 파일 작업만 진행한다.
- 동일 프로젝트 안에서 병렬 작업을 할 때는 파일/모듈 소유권을 먼저 나누고, 같은 파일을 두 세션이 동시에 수정하지 않는다.
- 루트 FluxOS 본부장을 통해 내려온 지시와 CEO가 다른 세션에 직접 내린 지시가 충돌하면, 직접 진행 중인 세션의 작업을 우선하고 본부장 세션은 해당 파일 작업을 보류한다.
- 상태 확인 없이 `git checkout`, `git reset`, `git restore`, 임의 덮어쓰기, 대량 포맷팅으로 다른 세션의 변경을 되돌리지 않는다.
