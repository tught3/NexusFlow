...
class SharedProductTablesService {
  ...
  static const Set<String> _sharedStatuses = <String>{
    'new',
    'wip',
    'done',
  };
  ...
  Future<void> submitBugReport({
    ...
    await _client.schema(schemaName).from('bug_reports').insert(<String, dynamic>{
      'product': product,
      'type': type,
  ...
  Future<void> submitFeedbackMessage({
    ...
    await _client.schema(schemaName).from('feedback_messages').insert(<String, dynamic>{
      'product': product,
      'name': name.trim(),
      'email': email.trim(),
  ...
  Future<void> submitEarlyBirdEmail({
    required String email,
  }) async {
    _requireNonEmpty(email, 'email');

    await _client.schema(schemaName).from('early_bird_emails').insert(<String, dynamic>{
      'product': product,
      'email': email.trim(),
    });
  }
...
final sharedProductTablesServiceProvider =
    Provider<SharedProductTablesService>((ref) {
